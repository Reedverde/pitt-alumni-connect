...
**2026-09-04 board status filters expanded.** The "Filter by" chips on `/` gained "Unclaimed" (state is unclaimed) and "No contact info" (unclaimed with `has_contact === false`), both single select and composable with the program filter and search. With a filter on, one sentence sits under the chips per `STATUS_BLURBS` in `src/routes/index.tsx`: Going says they said they are attending, Maybe asks you to encourage your teammates, Claimed says they verified their contact information, Unclaimed says they haven't checked in, No contact info asks you to click their name if you can reach them. The flat list header phrase and empty state copy cover the new codes.

**2026-09-04 unclaimed and no-contact filters chunk by five years.** With the Unclaimed or No contact info filter on, the result list groups into labeled five year buckets ("2025-2029 · 73") newest first, per the yearChunks grouping in `src/routes/index.tsx`. Going, Maybe, Claimed, and search results stay flat. Bucketing keys off `board_year`, so it is a query, not a stored value.

## /v2 experimental homepage (2026-09-04)
`/` is unchanged, visually and functionally. To share the claim board without forking it, the whole board experience (search, status and program filters, chips, claim dialog, contact tip dialog, person panel, counter bar, nav, footer) moved verbatim out of `src/routes/index.tsx` into `src/components/board/BoardExperience.tsx`, which exports `boardQuery` and takes two props: `renderHero` and `story`. `src/routes/index.tsx` now supplies its existing hero and story sections through those props, so the live homepage renders the same tree as before.

New route `src/routes/v2.tsx` (noindex) renders the same `BoardExperience` with `V2Hero` and `V2Story`. New files: `src/components/v2/ChamferPhoto.tsx` (deep 45 degree chamfer photo and colour field, lazy loading, reserved aspect ratio), `src/components/v2/V2Hero.tsx`, `src/components/v2/V2Story.tsx`. Added token `--royal-dark: #002A75` to `src/styles.css`. Gold stays reserved for attending: the only gold on /v2 is the existing eyebrow slash.

## /v2 photography revision (2026-09-04)
`/` still unchanged. Three changes, all scoped to `src/components/v2/`:

1. `ChamferPhoto` no longer applies the duotone filter. Photographs on /v2 render in original colour; legibility comes from composition and placement. The board's own `YearPhoto`, `PhotoSlot` and `SidelineLoop` keep their duotone because they are the shared board and must stay byte identical for `/`.
2. New `src/components/v2/curated-photos.ts` replaces the year based picker (`photos.ts`, deleted). It references real rows in the `photos` table by storage path: `ACTION` holds five action frames (throwing around a mark, skying for a catch, a close catch, a night run with the disc, a contested throw on turf), and `ALUMNI_WEEKEND` holds the three past Alumni Weekend photographs on file (2013 indoor, 2012, 2014 alumni games). No stock, no invented URLs.
3. Hero uses the around-the-mark and sky frames; the "We came back." block uses the night run, close catch and contested throw; the transition into the board uses the sky frame. The Three Days section now carries an asymmetric Friday / Saturday / Sunday collage built only from Alumni Weekend imagery, with varied ratios (4:3, 1:1, 16:10), alternating chamfer corners and staggered vertical offsets. Written summaries and the `/schedule` link are unchanged.
...
## /v2 hero rebuilt around the huddle photograph (2026-09-04)
`/` and the shared board unchanged; nothing published. Four user supplied photographs uploaded as CDN assets (`src/assets/hero-team-huddle.jpg.asset.json`, `hero-nationals-celebration`, `hero-clemson-on-our-bye`, `pitt-first-nationals-team-2005`). `V2Hero` is no longer a three photo collage: one oversized huddle image runs to the page edge under an aggressive asymmetric polygon clip (deep top left chamfer, bottom right cut), with a royal plane continuing the top diagonal behind it and a navy plane continuing the bottom cut beneath it. The headline sits beside the image. The "First 2 Weeks of October" seal straddles the photograph's top left chamfer, roughly half on and half off, clear of faces. `V2Story` now uses the Nationals celebration as the "We came back." image, the Clemson bye photograph as the culture crop, and adds a captioned archival block: the 2005 squad with the visible caption "The first Pitt team to reach Nationals · 2005". Original colour throughout, deep cuts kept. Verified at 1440 and 390: no new overflow (only the preexisting board chip), no console errors, typecheck and build clean.

## /v2 story consolidation and Three Days rounding (2026-09-04)
`/` unchanged. Scoped to `src/components/v2/V2Story.tsx`:
1. Removed the two orphaned image-only blocks (standalone contested-pile action photo and the tall Clemson-on-our-bye portrait on cream). The Clemson asset import is gone from /v2.
2. The 2005 first Nationals team photograph moved up into the primary "We came back" section; the lede, supporting copy, and the 2005 history note now form one two-column editorial composition around that photo. Corrected sentence: "2025 was the first year since 2005 we missed Nationals. Everyone felt it. Then this year, we made it back." Caption kept: "The first Pitt team to reach Nationals · 2005." The Nationals celebration photo remains as the supporting image beside the heading.
...
## /v2 hero badge moved to right (2026-09-04)
`/` unchanged. In `src/components/v2/V2Hero.tsx` the "First 2 Weeks of October · Every Year" seal now sits at the bottom-right chamfer of the dominant huddle image, with its right edge aligned to the photograph's right edge (`right-[-4vw]` on desktop, `right-[2%]` on mobile) so roughly half overlaps the photo and half extends onto the black background triangle created by the diagonal crop. It is clear of the huddle faces. A soft `drop-shadow(0 14px 28px rgba(0,0,0,0.55))` sits behind the badge to separate its gold edge from both the photograph and the dark field, with no glow. Centered logo navigation, huddle hero, removed script overlay, story composition, Three Days crops, and shared board/claim experience remain untouched. Verified at 1280 and 390; typecheck and build clean. Not published.

## Drip dispatcher hardening (2026-09-04)
The dispatcher stays a TanStack server module, not an edge function: `src/lib/drip.server.ts` plus `src/lib/drip-cron.server.ts`, invoked by pg_cron through `/api/public/hooks/drip-cron-tick` with the shared secret. Changes: the tick now refuses to do anything unless `app_settings.outbound_email_mode` reads exactly `drip_enabled`, and it restores that armed value after each dispatch instead of dropping the site to `transactional_only`. The audience now also drops anyone with `show_on_board = false` and anyone with a hard bounce or complaint on record, and the global per person cooldown moved from 7 to 10 days. The cron schedule moved from 00:00 UTC to 14:00 UTC, 9 AM Eastern. Sequence rows, copy builders, suppression checks, send logging and the one click unsubscribe header were already in place and unchanged. Note: `outbound_email_mode` is currently `drip_enabled`, so the next 14:00 UTC tick will send every overdue window.

## /v2 rounded chamfer system across every photograph (2026-09-04)

`/` unchanged. Every cut photograph on /v2 now uses `RoundedChamferPhoto`, so the angular polygon silhouettes stay but no vertex is a razor point. The component gained: an optional `points` prop (silhouette as fractions of the box) for non-uniform hero shapes, `eager` for the LCP image, `style`, and an exported `CHAMFER_RADIUS` scale (`hero 22, wide 16, portrait 14, small 12`) so radius stays proportionate to image size. The hero huddle image's asymmetric clip is now `HUDDLE_POINTS` fed through the same SVG mask; the Nationals celebration (portrait), 2005 archival photo (portrait) and the Clemson bye week transition photo (wide) moved off `ChamferPhoto`; Three Days keeps 8/3, notch 40, radius 12. Crops, ratios, colours, copy, navigation and the badge placement plus shadow are unchanged. Desktop 1280 and mobile 390 show no clipping; the only horizontal overflow on mobile is the pre-existing board filter chip scroller. Typecheck and build clean. Not published.

## Chamfer design tokens and shared primitives (2026-09-04)

The rounded-chamfer rule is now a tokenized system rather than per component numbers.

Tokens. `src/components/media/chamfer.ts` exports `CHAMFER`, a record of four tiers, each `{ cut, radius }` in px: `sm 20/8` (chips, thumbnails), `md 40/12` (standard content images, three-up rows, decorative planes), `lg 64/16` (feature portraits, wide editorial images), `hero 96/22` (page dominant photography and full bleed planes). `cut` is the depth of the 45 degree slice, `radius` softens every vertex of the resulting polygon. The same numbers are mirrored in `src/styles.css` as `--chamfer-{sm,md,lg,hero}-{cut,radius}` for CSS only surfaces. The board's older `--notch-sm` / `--notch-lg` and `components/media/notch.ts` are a separate system and stay untouched so `/` does not shift.

Helpers. `roundedPolygonPath(points, radius)` trims each vertex and inserts quadratic arcs, capped at half the shorter adjacent edge. `chamferPath({ w, h, cut, radius, corners, points })` resolves either a standard corner chamfer or a custom fractional polygon and clamps the cut to half the short side.

Primitives. `src/components/media/RoundedChamferBox.tsx` measures its box, builds the path and clips its children. It takes `tier` plus optional `cut`, `radius`, `corners`, `points`, `ratio`. `RoundedChamferField` wraps it for flat colour planes (`fill`, `opacity`), and `src/components/v2/RoundedChamferPhoto.tsx` wraps it for photographs (`src`, `alt`, `position`, `eager`). Absolute or fixed positioning on the caller's className is respected instead of being overridden.

Usage. Pass a tier, not literal geometry; reach for `cut`/`radius` only when a composition genuinely needs off scale values. `points` is for silhouettes that are not plain corner chamfers, such as the hero photograph and its two supporting planes.

/v2 refactor. Every angled surface now consumes the tokens: hero huddle photo and its royal and navy planes (`hero`, `hero`, `lg`), Nationals celebration and the 2005 archival photo (`lg`), the story navy plane (`hero`, top-left cut only), the Clemson bye week wide transition photo and the three weekend photographs (`md`). The old `src/components/v2/ChamferPhoto.tsx` is deleted; nothing referenced it any more. Ordinary rectangular or pill controls are unchanged, and `/` plus the shared board and claim flow render exactly as before. Verified at 1280 and 390 with no clipping or new overflow, the only horizontal overflow on mobile being the pre-existing filter chip scroller. Typecheck and build clean. Not published.

## Connected alumni redesign, Phase 1 data foundations (2026-09-04)

Database only. No page, component, route or design token was touched, so `/`, the claim dialog, `/me` and `/admin` render exactly as before, and the chamfer tokens are untouched.

Added. `events.published boolean not null default true`, orthogonal to `is_placeholder` (timing not locked) and `prompt_rsvp` (collects its own headcount); all five 2026 events are published, so current intent is preserved. `rsvp_editable_until(year)` resolves the close moment as: organizer override in `app_settings.rsvp_editable_until` (either a bare timestamp or `<year>=<timestamp>`), else the latest `ends_at` across that edition's published events, else 23:59:59 on `editions.ends_on` in America/New_York, whichever is later. No grace period. `rsvp_is_editable(year)` is the boolean form. Both are security definer, revoked from anon and PUBLIC, granted to authenticated and service_role. Only two of the five 2026 events carry an `ends_at`, which is exactly why the edition fallback exists; today the rule resolves to end of Sunday Oct 4 Eastern.

`board_coaches` now filters `archived = false`, matching `board_people`. Nothing else in the view changed. `roster_stints` is the honestly named reading of the trusted-source stint data that `current_players` misdescribes; same 152 rows, `security_invoker = on`, and `current_players` stays in place with a deprecation comment so existing readers keep working.

Reporting. `admin_annual_rsvp_totals(year)` returns going, maybe, not this year, no response and planned heads. `admin_event_rsvp_totals(year)` returns yes, no, unanswered and planned heads per published prompt event. Both raise `admin only` through `is_admin()`, are security definer, revoked from anon and PUBLIC. "No response" is derived as eligible people minus rows in `rsvps`, so it stays distinct from an explicit not this year.

Contact tips. `suggestions.payload` for `contact_tip` now carries `contact_type` alongside `contact_value`, backfilled by shape into email, phone or other. There are currently no `contact_tip` rows, so the backfill was a no-op; the single existing suggestion is a `new_person` row and was not touched. Tips remain admin and owner only; no view exposes an address.
...
- Resolved: `event_rsvp_prompt` already sent on 2026-09-04 (63 rows in `sends`), so the once-per-campaign rule would have suppressed everyone. It is now `active = false` and historical. The approved September 22 send has its own identity: key **`event_rsvp_prompt_t10_2026_09_22`** (`915b8152-cd97-4816-b17a-db9e57117e26`), `offset_days = -10`, `active = true`, audience `{going}`, `one_time = false`, target **2026-09-22**.
- The new key reuses the September 4 copy and builder unchanged (`EVENT_RSVP_PROMPT_KEYS` in `src/lib/drip.server.ts` routes both keys to `buildEventRsvpPromptBody` and to `loadPendingEvents`). The global 10-day cooldown is untouched; September 4 sends are 18 days old by the target date, so they no longer block.
- The daily cron reconsiders any active sequence whose target date has passed, which is harmless here: per-person `already_sent` and per-address dedupe make every run after the first a no-op, and each run still writes a `drip_cron_tick` row to `audit_log`.
- Target-date projection on 2026-09-05 data: 64 eligible Going, 57 would send, 7 no-body (every event answered), 0 already-sent, 0 cooldown, 0 duplicate address, 0 unreachable/suppressed.

## 2026 current-roster classification (2026-09-05)

- "Current player" is derived, never stored on a person: a `stints` row with `role in ('player','captain')` for the roster season (`board_people.is_current`, `admin.server.ts` `rosterSeasonYear()`). Coaches are not current players.
- Sources of record for 2026: **MENS_B / Pressure** — the roster captain Jared Kartschoke gave Reed on 2026-09-05 (31 players, coaches Ben Banyas, Will Litchholt, Tyler Weinberger). **MENS_A / En Sabah Nur** — `https://www.esnultimate.org/history/season/?season=2026` ("History: 2025–2026 Season"), all roster tables count as current except the 5th Year/Grad section (Adam Ricci, Jake Sperry, Peter Kotz), per Reed's 2026-09-05 direction. A coaches unchanged: Nick Kaczmarek, Dylan Best.
- Applied to 2026 only: nine MENS_B playing stints removed (Alex Lemoncelli, Benjamin Green, Christian Harper, Keller Ulrich, Michael Lowry, Quinn Siegel, Ronan Murphy, Tommy Selwood, Yun Dong), Tyler Weinberger added as MENS_B coach, 24 MENS_A player stints added. No person, name, RSVP, event RSVP, send or suppression touched; older stints untouched.
- Every removal and addition has an `audit_log` row (`roster_stint_removed_2026_mens_b`, `roster_stint_added_2026_mens_b`, `roster_stint_added_2026_mens_a`) carrying the person's name and the source evidence.
- Provenance note: the request asked for `source = 'admin'`, but `block_current_season_playing_stint` admits current-season playing stints only under `source = 'roster_import'`. The trigger was left intact and player rows were written as `roster_import`; the intended `admin` provenance and the full evidence live in `audit_log`. The Tyler Weinberger coach row is `source = 'admin'` (coach roles are not trigger-protected).
- Post-update 2026 totals: going 64 people / 79 heads (41 alumni / 56 heads, 23 current players / 23 heads); maybe 38 (34 alumni, 4 current); not this year 52 (49 alumni, 3 current). Board shows 55 current (31 B + 24 A).
