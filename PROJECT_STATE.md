...
- "Current" has one definition, the `current_people` view: any stint in the newest roster year present in `stints` (currently **2027**), or a working `@pitt.edu` identity — an address with a logged send carrying a provider message id and no suppression. `identities.verified_at` is untouched and still means inbox authentication only. `board_people.is_current` and the organizer people/head splits (`admin.server.ts` `loadContext`) both read that view; no other place computes it.
