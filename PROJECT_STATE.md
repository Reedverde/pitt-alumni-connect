...
New route `src/routes/v2.tsx` (noindex) renders the same `BoardExperience` with `V2Hero` and `V2Story`. New files: `src/components/v2/ChamferPhoto.tsx` (deep 45 degree chamfer photo and colour field, lazy loading, reserved aspect ratio), `src/components/v2/photos.ts` (picks real photographs off `photosByYear`, never invents a URL), `src/components/v2/V2Hero.tsx`, `src/components/v2/V2Story.tsx`. Added token `--royal-dark: #002A75` to `src/styles.css`. Gold stays reserved for attending: the only gold on /v2 is the existing eyebrow slash.
