DROP VIEW IF EXISTS public.board_coaches;

CREATE VIEW public.board_coaches
...
SELECT p.id,
 ...
 WHERE p.show_on_board = true
   ...
   AND NOT EXISTS (SELECT 1 FROM stints s WHERE s.person_id = p.id AND s.role NOT IN ('coach'::text, 'manager'::text));

GRANT SELECT ON public.board_coaches TO anon, authenticated;