...
SELECT p.id,
    ...
    EXISTS (
      ...
      WHERE s.person_id = p.id
        AND s.role = ANY (ARRAY['player'::text, 'captain'::text])
        AND s.year = (EXTRACT(year FROM (now() AT TIME ZONE 'America/New_York'))::int)
    ) AS is_current
   FROM people p
...
GRANT SELECT (id, first_name, last_name, played_as, deceased, board_year, board_division, team_label, state, is_current) ON public.board_people TO anon, authenticated;
